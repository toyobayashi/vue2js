...
class Parser {
  ...
  _append (templateResult, v, hasFunctional, hasScoped, id, exportVar) {
    const optionVar = `__options_${id}`
    return `;(function () {
${exportVar ? `var ${v} = ${exportVar};` : ''}
var ${optionVar} = typeof ${v} === 'function' ? ${v}.options : ${v};
...
export default ${v};`
      ...
      code = `${content}
${this._append(templateResult, v, hasFunctional, hasScoped, id, moduleExports)}`
